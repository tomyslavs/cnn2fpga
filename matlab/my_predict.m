...
for n=1:length(x(1,1,1,:))
    ...
    %% --------------------------------------------------------------------- %%
    %% 2 Conv.
    w2 = net.Layers(5).Weights;             w2 = rot90(w2,2);
    ...
    %% 2 Batch norm.
    C2_xmu = C2s - net.Layers(6).TrainedMean; 
    C2_sqrtvar = sqrt(net.Layers(6).TrainedVariance + net.Layers(6).Epsilon);
    C2_ivar = 1./C2_sqrtvar;
    C2_xhat = C2_xmu .* C2_ivar;
    C2_gammax = net.Layers(6).Scale .* C2_xhat;
    C2n = C2_gammax + net.Layers(6).Offset;
    %% 2 ReLU
    C2nLReLU = max(0,C2n) + min(0,C2n)*net.Layers(7).Scale; % LeakyReLU pos + neg*scale

    %% --------------------------------------------------------------------- %%
    %% 3 Conv.
    w3 = net.Layers(8).Weights;             w3 = rot90(w3,2);
    ...
    %% 3 Batch norm.
    C3_xmu = C3s - net.Layers(9).TrainedMean;
    C3_sqrtvar = sqrt(net.Layers(9).TrainedVariance + net.Layers(9).Epsilon);
    C3_ivar = 1./C3_sqrtvar;
    C3_xhat = C3_xmu .* C3_ivar;
    C3_gammax = net.Layers(9).Scale .* C3_xhat;
    C3n = C3_gammax + net.Layers(9).Offset;
    %% 3 ReLU
