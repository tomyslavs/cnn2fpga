...
options = set_net_options(10,100,0.0001,'no',XValidation,YValidation); % 0.00002
...
%% Train and convert to sfixed iteration by iteration
FractionLength = 15; % in bits
options = set_net_options_fixed(10,1,0.003,'no',XValidation,YValidation); % 0.00002
fc_w = [];
for i=1:5000
    net = trainNetwork(XValidation,YValidation,layers,options); % layers or net.Layers % XTrain,YTrain
    net = bin_conv_weights_fixed(net,'yes',FractionLength); % Binarize conv weights
%     net = bn_fixed(net,FractionLength); % BN layers
    net = fc_fixed(net,FractionLength); % FC layers
    layers = net.Layers; % update conv, bn, fc layers with sfixed parameters
    
%     tmp = net.Layers(11).Weights;
%     fc_w = cat(2,fc_w,tmp(1,1));
%     figure(1);
%     plot(fc_w); grid on;
