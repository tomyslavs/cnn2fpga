...
idx = 11;
Img = XTest(:,:,:,idx);
% Img = cat(3,Img,XTest(:,:,:,idx));
% Img = cat(3,Img,XTest(:,:,:,idx));
imwrite(uint8(Img*256),'C:\CNN_VHDL\python\fir.bmp'); % FPGA input image
...
figure(1000); plot(YPredicted0,'b'); hold on; %title('FPGA FC2');
% plot(fc2out,'g'); hold on; % FixedMatlab
% plot(FC2,'g'); hold on; % Float2
plot(FC2_FPGA,'r--');
legend Float FPGA; grid on; ylabel('Amplitude'); xlabel('Neurono indeksas'); hold off;
Mdiff=mean(abs(FC2_FPGA-YPredicted0));
disp(['Mean error ', num2str(Mdiff)]);

figure(idx); imshow((Img));
%% Display markers
